...
def mainCMake():
    ...
    if options.cmake:
        ...
        if options.kdevelop:
            sMode = "Debug"
            gen = "-G \"KDevelop3 - Unix Makefiles\""
        elif options.eclipse:
            sMode = "Debug"
            gen = "-G \"Eclipse CDT4 - Unix Makefiles\""
        run("cd %s; cmake %s -DNOSTRIP=yes -DCMAKE_BUILD_TYPE=%s %s ../arcadia" % (buildHome, makeOnly, sMode, gen))

    threadCount = int(options.thread)
    makeOnly = ""
    if not options.all:
        makeOnly = projectParts
    if not options.kdevelop and not options.eclipse:
        make(MAKE_OPTIONS, newCwd, threadCount, makeOnly, options.tests, buildHome)
    elif options.kdevelop:
